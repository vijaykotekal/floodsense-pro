...
function FloodSenseApp() {
  ...
  const [role, setRole] = useState<"Citizen" | "Authority" | "Administrator">("Authority");
  ...
  if (!hasEntered) return <><Toaster position="bottom-right" /><EntryScreen onEnter={(selectedRole) => { setRole(selectedRole); setHasEntered(true); }} /></>;

  return (
    <div className="min-h-screen bg-paper font-sans text-ink antialiased">
      <Toaster position="bottom-right" />
      <div className="mx-auto flex min-h-screen max-w-[1600px]">
        <Sidebar view={view} role={role} onNavigate={setView} />
        {mobileNav && <MobileNav view={view} onNavigate={(next) => { setView(next); setMobileNav(false); }} onClose={() => setMobileNav(false)} />}
        <main className="min-w-0 flex-1">
          <Topbar role={role} onMenu={() => setMobileNav(true)} onNotifications={() => setNotificationsOpen((open) => !open)} notificationsOpen={notificationsOpen} />
...
function EntryScreen({ onEnter }: { onEnter: (role: "Citizen" | "Authority" | "Administrator") => void }) {
  const [role, setRole] = useState<"Citizen" | "Authority" | "Administrator">("Authority");
  const [signUp, setSignUp] = useState(false);
  const enter = () => { toast.success(`Demo ${role.toLowerCase()} access ready`); onEnter(role); };
  return <main className="grid min-h-screen place-items-center bg-paper px-4 py-10"><div className="grid w-full max-w-5xl overflow-hidden border border-line bg-panel md:grid-cols-[1.1fr_0.9fr]"><section className="relative hidden flex-col justify-between border-r border-line bg-panel2 p-10 md:flex"><div><div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-md bg-signal font-display text-lg font-bold text-signalink">F</div><div><div className="font-display text-lg font-bold">FloodSense AI</div><div className="font-mono text-[10px] text-dim">EARLY WARNING SYSTEM · DEMO</div></div></div><div className="mt-20 max-w-md"><div className="font-mono text-[11px] uppercase tracking-[0.18em] text-signal">AI-BASED MOUNTAIN FLOOD & FLASH-FLOOD EARLY WARNING SYSTEM</div><h1 className="mt-4 font-display text-5xl font-bold leading-tight">Predict.<br />Protect.<br /><span className="text-signal">Prepare.</span></h1><p className="mt-5 max-w-sm text-sm leading-relaxed text-dim">A project demonstration for risk prediction, clear explanations, early warning, evacuation planning, and environmental restoration.</p></div></div><div className="flex items-center justify-between border-t border-line pt-4 font-mono text-[10px] text-dim"><span>SIMULATED DATA · NO LIVE EMERGENCY SERVICE</span><span>v0.4</span></div></section><section className="p-6 sm:p-10"><div className="mb-8 md:hidden"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-md bg-signal font-display font-bold text-signalink">F</div><div><div className="font-display font-bold">FloodSense AI</div><div className="font-mono text-[9px] text-dim">DEMO ENVIRONMENT</div></div></div></div><div className="font-mono text-[10px] uppercase tracking-widest text-signal">DEMO ACCESS</div><h2 className="mt-2 font-display text-3xl font-bold">{signUp ? "Create demo profile" : "Welcome back"}</h2><p className="mt-2 text-sm text-dim">Sign in or continue with a role-based demo account.</p><form className="mt-7 space-y-4" onSubmit={(event) => { event.preventDefault(); enter(); }}><label className="block"><span className="mb-1.5 block font-mono text-[10px] uppercase tracking-wider text-dim">Email</span><input type="email" placeholder="name@example.org" required className="h-10 w-full border border-line bg-paper px-3 text-sm outline-none focus:border-signal" /></label><label className="block"><span className="mb-1.5 block font-mono text-[10px] uppercase tracking-wider text-dim">Password</span><input type="password" placeholder="Enter password" required className="h-10 w-full border border-line bg-paper px-3 text-sm outline-none focus:border-signal" /></label><label className="block"><span className="mb-1.5 block font-mono text-[10px] uppercase tracking-wider text-dim">Demo role</span><select value={role} onChange={(event) => setRole(event.target.value as typeof role)} className="h-10 w-full border border-line bg-paper px-3 text-sm text-ink outline-none focus:border-signal"><option>Citizen</option><option>Authority</option><option>Administrator</option></select></label><Button type="submit" className="h-10 w-full bg-signal font-mono text-xs font-semibold text-signalink hover:bg-signal/90">{signUp ? "SIGN UP · DEMO" : "LOGIN · DEMO"}</Button></form><Button variant="outline" className="mt-3 w-full" onClick={enter}>Continue as Demo User <ChevronRight /></Button><div className="mt-5 flex items-center justify-between border-t border-line pt-4 text-xs"><span className="text-dim">{signUp ? "Already registered?" : "New to the demo?"}</span><button className="font-medium text-signal" onClick={() => setSignUp((value) => !value)}>{signUp ? "Login" : "Sign Up"}</button></div><p className="mt-6 text-[10px] leading-relaxed text-faint">Demo login only. No account is created, and no credentials are transmitted or stored.</p></section></div></main>;
}

function Sidebar({ view, role, onNavigate }: { view: View; role: string; onNavigate: (view: View) => void }) {
...
function AnalyticsView({ period, setPeriod }: { period: keyof typeof trendData; setPeriod: (period: keyof typeof trendData) => void }) {
  const sourceData: Array<{ label: string; text: string; icon: LucideIcon }> = [
    { label: "Meteorological", text: "Rainfall · humidity · cloudburst", icon: CloudRain },
    { label: "Geographical", text: "Elevation · slope · terrain", icon: Mountain },
    { label: "Environmental", text: "Vegetation · forest cover", icon: TreePine },
    { label: "Hydrological", text: "River · flow · dam levels", icon: Droplets },
    { label: "Human activity", text: "Construction · land-use", icon: Users },
  ];
  const chartData = [["Rainfall over time", 82, "bg-signal"], ["River level over time", 68, "bg-high"], ["Vegetation coverage", 31, "bg-safe"], ["Alert frequency", 55, "bg-water"]] as const;
  return <><PageHeading eyebrow="DEMO DATASET · MULTI-SOURCE" title="Data & Analytics" description="Inspect the source categories that feed the modeled risk score and compare trends over time." /><div className="grid gap-4 lg:grid-cols-2"><RiskTrend period={period} setPeriod={setPeriod} /><section className="rounded-lg border border-line bg-panel p-4"><div className="font-display font-semibold">Rainfall, river, and vegetation</div><div className="mt-5 space-y-4">{chartData.map(([label, value, color]) => <div key={label}><div className="flex justify-between font-mono text-[10px] text-dim"><span>{label}</span><span>{value}%</span></div><div className="mt-1.5 h-3 rounded-full bg-line"><div className={`h-full rounded-full ${color}`} style={{ width: `${value}%` }} /></div></div>)}</div></section></div><section className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{sourceData.map(({ label, text, icon: Icon }) => <div key={label} className="rounded-lg border border-line bg-panel p-4"><Icon className="size-4 text-signal" /><div className="mt-4 font-display font-semibold">{label}</div><div className="mt-1 text-xs leading-relaxed text-dim">{text}</div></div>)}</section></>;
}

function AuthoritiesView({ onNavigate }: { onNavigate: (view: View) => void }) {
  const kpis: Array<{ label: string; value: string; icon: LucideIcon; color: string }> = [
    { label: "Active high-risk areas", value: "5", icon: AlertTriangle, color: "high" },
    { label: "People potentially affected", value: "3.2k", icon: Users, color: "signal" },
    { label: "Open shelters", value: "8", icon: ShieldCheck, color: "safe" },
    { label: "Pending alerts", value: "3", icon: Bell, color: "high" },
  ];
  const actions: Array<{ label: string; icon: LucideIcon; target: View }> = [
    { label: "Send Alert", icon: Bell, target: "Alerts" },
    { label: "Open Evacuation Plan", icon: RouteIcon, target: "Evacuation" },
    { label: "View Risk Map", icon: Map, target: "Risk Map" },
    { label: "Assign Response Team", icon: Users, target: "Authorities" },
    { label: "Update Shelter", icon: ShieldCheck, target: "Evacuation" },
    { label: "Mark Incident Resolved", icon: Check, target: "Authorities" },
  ];
  return <><PageHeading eyebrow="AUTHORITY VIEW · DEMO RESPONSE" title="Emergency Overview" description="Coordinate simulated response actions across high-risk areas, shelters, alerts, and incident status." /><div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{kpis.map(({ label, value, icon: Icon, color }) => <div key={label} className="rounded-lg border border-line bg-panel p-4"><Icon className={`size-4 text-${color}`} /><div className="mt-4 font-display text-3xl font-bold">{value}</div><div className="mt-1 text-xs text-dim">{label}</div></div>)}</div><div className="mt-4 grid gap-4 lg:grid-cols-3"><section className="rounded-lg border border-line bg-panel p-4 lg:col-span-2"><div className="flex items-center justify-between"><div className="font-display font-semibold">Incident management</div><span className="font-mono text-[10px] text-signal">INCIDENT M-04</span></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[620px] text-left text-xs"><tbody>{[["Location", "Upper Kumaun Basin"], ["Risk level", "HIGH · 78/100"], ["Detected time", "08:04 IST"], ["Assigned authority", "District Response Team 04"], ["Response status", "Response Activated"], ["Notes", "Verify evacuation plans and shelter capacity"]].map(([label, value]) => <tr key={label} className="border-b border-line/60"><td className="w-1/3 px-3 py-3 font-mono text-[10px] uppercase tracking-wider text-faint">{label}</td><td className="px-3 py-3 text-dim">{value}</td></tr>)}</tbody></table></div></section><section className="rounded-lg border border-line bg-panel p-4"><div className="font-display font-semibold">Response actions</div><div className="mt-4 space-y-2">{actions.map(({ label, icon: Icon, target }) => <Button key={label} variant="outline" className="w-full justify-start" onClick={() => { if (target === "Authorities") toast.success(`${label} demo action recorded`); else onNavigate(target); }}><Icon />{label}</Button>)}</div></section></div></>;
}

function SettingsView({ role }: { role: string }) { return <><PageHeading eyebrow="SYSTEM CONFIGURATION · DEMO" title="Settings" description="Prototype configuration and role access for the academic demonstration environment." /><div className="grid gap-4 lg:grid-cols-2"><section className="rounded-lg border border-line bg-panel p-5"><div className="font-display font-semibold">Demo mode</div><div className="mt-4 flex items-center justify-between border-b border-line pb-4"><div><div className="text-sm">Simulated data and model output</div><div className="mt-1 text-xs text-dim">No external sensors, government feeds, or live dispatch are connected.</div></div><div className="rounded-full bg-safe/15 px-3 py-1 font-mono text-[10px] text-safe">ON</div></div><div className="mt-4 space-y-3 text-sm text-dim"><div className="flex justify-between"><span>Active role</span><span className="text-ink">{role}</span></div><div className="flex justify-between"><span>Dataset</span><span className="text-ink">Mountain region sample</span></div><div className="flex justify-between"><span>Prediction service</span><span className="text-safe">Demo formula ready</span></div></div></section><section className="rounded-lg border border-line bg-panel p-5"><div className="font-display font-semibold">Notification channels</div><div className="mt-4 space-y-3">{["SMS", "Mobile notification", "Messaging", "Authority dashboard"].map((channel) => <label key={channel} className="flex items-center justify-between border-b border-line/60 pb-3 text-sm text-dim"><span>{channel}</span><input type="checkbox" defaultChecked className="accent-signal" /></label>)}</div></section></div></>; }

type LeafletModules = typeof import("react-leaflet");
function LeafletMap({ route, className }: { route: boolean; className: string }) {
  const [leaflet, setLeaflet] = useState<LeafletModules | null>(null);
  useEffect(() => {
    let active = true;
    void import("react-leaflet").then((modules) => { if (active) setLeaflet(modules); });
    return () => { active = false; };
  }, []);
  if (!leaflet) return <div className={`demo-map ${className} grid place-items-center rounded-md font-mono text-[10px] text-dim`}>LOADING DEMO GIS LAYERS…</div>;
  const { MapContainer, TileLayer, Circle, CircleMarker, Polyline, Popup } = leaflet;
  const path: [number, number][] = [[29.92, 79.31], [29.924, 79.323], [29.93, 79.334], [29.938, 79.344]];
  return <div className={`leaflet-frame relative overflow-hidden rounded-md ${className}`}><MapContainer center={[29.92, 79.31]} zoom={12} scrollWheelZoom={false} style={{ height: "100%", width: "100%" }}><TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" /><Circle center={[29.925, 79.29]} radius={900} pathOptions={{ color: "var(--color-high)", fillColor: "var(--color-high)", fillOpacity: 0.22 }}><Popup><strong>Upper Kumaun Basin</strong><br />HIGH · score 78<br />Rainfall 124 mm · River 4.8 m<br />Demo risk zone. Verify locally.</Popup></Circle><Circle center={[29.91, 79.32]} radius={1400} pathOptions={{ color: "var(--color-signal)", fillColor: "var(--color-signal)", fillOpacity: 0.12 }}><Popup>Moderate-risk zone · Demo data</Popup></Circle><Polyline positions={path} pathOptions={{ color: "var(--color-water)", weight: 4, opacity: 0.75 }} /><CircleMarker center={path[0]} radius={8} pathOptions={{ color: "white", fillColor: "var(--color-water)", fillOpacity: 1 }}><Popup>Your location · demo coordinate</Popup></CircleMarker><CircleMarker center={path[3]} radius={8} pathOptions={{ color: "white", fillColor: "var(--color-safe)", fillOpacity: 1 }}><Popup>Community Shelter A · 142 spaces available</Popup></CircleMarker><CircleMarker center={[29.91, 79.34]} radius={6} pathOptions={{ color: "white", fillColor: "var(--color-signal)", fillOpacity: 1 }}><Popup>Dam K-2 · water level 82%</Popup></CircleMarker>{route && <Polyline positions={path} pathOptions={{ color: "var(--color-safe)", weight: 5, dashArray: "8 7" }} />}</MapContainer><div className="pointer-events-none absolute bottom-3 left-3 z-[500] rounded-md bg-panel/90 p-2 font-mono text-[10px] text-dim">DEMO GIS · OpenStreetMap base layer</div></div>;
}
